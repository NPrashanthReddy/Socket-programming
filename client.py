        ...
        with open(self.filename,"rb") as f:
            while True:
                ...
                if not bytes_read:
                    ...
                    break
                #we use sendall to ensure that all bytes are sent
    ...
    def write(self):
        ...
        if str[0:2] == '>>':
            ...
            if self.gui_done: #makes sure our building of GUI is done
                    ...
                    self.text_area.insert('end', f"{self.filename} has been sent\n") #append the message at the end
        ...
        while self.running:
            try: 
                ...
                if message == "USERNAME":
                    ...
                    msg=tkinter.Tk()
                    msg.withdraw()
                    msggg=simpledialog.askstring(f"Do you want to receive the file {filename}", "Type yes or no\t\t\t\t\t\t\t\t", parent=msg)
                    accept_file=False
                    ...
                    if accept_file:
                        ...
                        if self.gui_done: #makes sure our building of GUI is done
                            self.text_area.config(state='normal')
                            self.text_area.insert('end', f"{filename} has been received\n") #append the message at the end
                            self.text_area.yview('end') #scroll your view along with messages
                            self.text_area.config(state='disabled')
                    else:
                        bytes_read = self.sock.recv(BUFFER_SIZE)
                        if self.gui_done: #makes sure our building of GUI is done
                            self.text_area.config(state='normal')
                            self.text_area.insert('end', f"{filename} transfer has been rejected\n") #append the message at the end
                            self.text_area.yview('end') #scroll your view along with messages
                            self.text_area.config(state='disabled')
